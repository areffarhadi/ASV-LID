...
Based on BUT paper: "BUT SYSTEMS FOR WILDSPOOF CHALLENGE: SASV IN THE WILD"
================================================================================
...
from dataset_asv import SpoofCelebASV, WavBatchSampler
...
# WaveletBlock + WPTW2VBERTMultiLayer are shared from peft_wpt.py so that the
# --peft_mode (deep_prompt / shallow_prompt / prefix) and --use_wavelet options
# are available to every entry point that imports this module (e.g. the TidyLang
# LID trainer main_train_lid.py). Default = deep_prompt + wavelet = original.
...
class SimpleSVModelWPTW2VBERTMHFA(nn.Module):
    ...
    def __init__(self, model_dir, num_speakers, embedding_dim=256, 
                 ...
                 head_dropout=0.1, use_arcface=True, arcface_margin=0.3, arcface_scale=30.0):
        super().__init__()
        ...
        if (epoch + 1) % args.interval == 0:
            ...
            if args.trial_file and os.path.exists(args.trial_file):
                ...
                for fn, emb in zip(val_filenames, val_embeddings):
                    ...
                    filename_to_emb[basename] = emb.numpy()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='WPT + W2V-BERT-2.0 + MHFA for Simple SV')
